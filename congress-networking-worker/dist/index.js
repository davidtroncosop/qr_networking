...
userRoutes.get("/:userId/interests", async (c) => {
  try {
    ...
    `).bind(userId).all();
    const interests = results.results?.map((r) => r.interest) || [];
    return c.json({
      success: true,
      interests
    });
...
eventRoutes.post("/", requireAuth2, async (c) => {
  try {
    const userId = c.get("userId");
    const data = await c.req.json();
    if (!data.name || !data.name.trim()) {
...
eventRoutes.put("/:eventId", requireAuth2, async (c) => {
  try {
    const eventId = c.req.param("eventId");
    const userId = c.get("userId");
    const event = await c.env.DB.prepare(`
...
eventRoutes.get("/:eventId/attendees", async (c) => {
  try {
    ...
    `).bind(eventId).all();
    const attendees = results.results?.map((row) => ({
      id: row.id,
      name: row.name,
      email: row.email,
...
eventRoutes.post("/:eventId/register", requireAuth2, async (c) => {
  try {
    const eventId = c.req.param("eventId");
    const userId = c.get("userId");
    const event = await c.env.DB.prepare(`
...
eventRoutes.post("/:eventId/quick-register", async (c) => {
  try {
    ...
    const interestsResult = await c.env.DB.prepare(`
      SELECT interest FROM user_interests WHERE user_id = ?
    `).bind(user.id).all();
    const interests = interestsResult.results?.map((r) => r.interest) || [];
    return c.json({
      success: true,
      message: "Successfully registered to event",
      data: {
...
connectionRoutes.post("/", requireAuth3, async (c) => {
  try {
    const userId1 = c.get("userId");
    ...
    if (existing) {
      const scannedUser2 = await c.env.DB.prepare(`
        SELECT * FROM users WHERE id = ?
      ...
      return c.json({
        ...
        connectedUser: {
          id: scannedUser2.id,
          name: scannedUser2.name,
          email: scannedUser2.email,
          phone: scannedUser2.phone,
          jobTitle: scannedUser2.job_title,
          company: scannedUser2.company,
          photoUrl: scannedUser2.photo_url,
          linkedinUrl: scannedUser2.linkedin_url,
          twitterUrl: scannedUser2.twitter_url,
          instagramUrl: scannedUser2.instagram_url,
          websiteUrl: scannedUser2.website_url
        }
      });
    }
...
connectionRoutes.get("/my-connections", requireAuth3, async (c) => {
  try {
    const userId = c.get("userId");
